...
public class HeatMap {
	...
	static final Map<PositionEnvironment, Vertex<PositionEnvironment, Integer>> GRAPH;
	...
	static {
		...
		for (int i = 1; i < SEGMENTS.length; ++i) { //TODO fix when segments are reindexed
			...
			if (s.startCrossing == null) {
				if (s.starts.length == 0) {
					s.startCrossing = new PositionEnvironment(i, 0);
					...
					if (Utils.contains(other.starts, i)) {
						if (other.startCrossing != null) {
							...
							if (s.x1 == other.x2 || s.y1 == other.y2) { //our start
								...
								s.startCrossing = new PositionEnvironment(s.starts[0], 0);
							} else {
						...
						if (other.endCrossing != null) {
							...
							if (s.x1 == other.x1 || s.y1 == other.y1) { //our start
								...
								s.startCrossing = new PositionEnvironment(s.starts[0], other.temps.length - 1);
							} else {
								throw new RuntimeException("This should never happen! Probably badly defined segments!");
							}
			...
			if (s.endCrossing == null) {
				if (s.ends.length == 0) {
					s.endCrossing = new PositionEnvironment(i, s.temps.length - 1);
					...
					if (Utils.contains(other.starts, i)) {
						if (other.startCrossing != null) {
							...
							if (s.x2 == other.x2 || s.y2 == other.y2) { //our end
								...
								s.endCrossing = new PositionEnvironment(s.ends[0], 0);
							} else {
								throw new RuntimeException("This should never happen! Probably badly defined segments!");
							}
						...
						if (other.endCrossing != null) {
							...
							if (s.x2 == other.x1 || s.y2 == other.y1) { //our end
								...
								s.endCrossing = new PositionEnvironment(s.ends[0], other.temps.length - 1);
							} else {
								throw new RuntimeException("This should never happen! Probably badly defined segments!");
							}
		...
		for (int i = 1; i < SEGMENTS.length; ++i) { //TODO fix when segments are reindexed
			...
			Vertex<PositionEnvironment, Integer> start = GRAPH.get(s.startCrossing);
			if (start == null) {
				start = new Vertex<>(s.startCrossing);
				GRAPH.put(start.value, start);
			}
			Vertex<PositionEnvironment, Integer> end = GRAPH.get(s.endCrossing);
			if (end == null) {
				end = new Vertex<>(s.endCrossing);
				GRAPH.put(end.value, end);
			}
			final Edge<PositionEnvironment, Integer> edge1 = new Edge<>(end, w, i);
			start.adjacencies.add(edge1);
			final Edge<PositionEnvironment, Integer> edge2 = new Edge<>(start, w, i);
...
//		}
	...
	public static double temperature(final PositionEnvironment position) {
//		try {
...
//		}
	...
	public static class Segment {
		...
		public PositionEnvironment startCrossing;

		/** Crossing at the start. */
		public PositionEnvironment endCrossing;
